    ...
    IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, IPointerMoveHandler
{
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        {
            onDragBegin?.Invoke(EIndex);     // 델리게이트로 알람 보내기
        }
    }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        {
            // 마우스 위치에 어떤 오브젝트가 있다.
            InvenSlotUI invenSlot = obj.GetComponent<InvenSlotUI>();  // 마우스 위치에 있는 오브젝트가 슬롯UI인지 확인
            EquipSlotUI equipSlot = obj.GetComponent<EquipSlotUI>();    // 마우스 위치에 있는 오브젝트가 장비 슬롯UI 인지 확인
            if (invenSlot != null)
            {
                // 슬롯UI다.
                invenSlot.onDragEnd?.Invoke(invenSlot.Index, true); // 끝난지점에 있는 슬롯의 인덱스와 정상적으로 끝났다고 알람 보내기
            }
        ...
        {
            Debug.Log("슬롯UI가 아니다.");
            onDragEnd?.Invoke(EIndex, false);        // 원래 드래그가 시작한 인덱스와 비정상적으로 끝났다고 알람 보내기
        }
    }
